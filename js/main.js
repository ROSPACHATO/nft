...
const items = document.querySelectorAll(".ask__acc-item");
items.forEach(item => {
...
const choices = new (choices_js__WEBPACK_IMPORTED_MODULE_0___default())("select[name='select']", {
  searchEnabled: false,
  shouldSort: false
});
...
let totalPrice = document.querySelector(".calc__res");
function calc() {
  const base = totalPrice.innerText = parseInt(num.value) * parseFloat(select.value);
  if (isNaN(base)) {
    totalPrice.innerText = 0 + "$";
  } else {
    totalPrice.innerText = parseInt(num.value) * parseFloat(select.value) + "$";
  }
}
...
/* harmony import */ var _components_choises__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./components/choises */ "./src/js/components/choises.js");
//Библиотеки
